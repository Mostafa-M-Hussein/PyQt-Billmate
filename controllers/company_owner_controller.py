...
class CompanyOwnerController(BaseController):
    ...
    def update_or_add_company(self, row, db_values):
        ...
        coupon_code = (
            self.view.table_widget.item(row, 6).data(Qt.DisplayRole)
            if self.view.table_widget.item(row, 6)
            else None
        )
        coupon_discount = (
            self.view.table_widget.item(row, 6).data(Qt.UserRole)
            if self.view.table_widget.item(row, 6)
            else None
        )

        shipping_name = (
            self.view.table_widget.item(row, 10).data(Qt.DisplayRole)
            if self.view.table_widget.item(row, 10)
            else None
        )
        shipping_percentage = (
            self.view.table_widget.item(row, 10).data(Qt.UserRole)
            if self.view.table_widget.item(row, 10)
            else None
        )

        payment_name = (
            self.view.table_widget.item(row, 7).data(Qt.DisplayRole)
            if self.view.table_widget.item(row, 7)
            else None
        )
        payment_percentage = (
            self.view.table_widget.item(row, 7).data(Qt.UserRole)
            if self.view.table_widget.item(row, 7)
            else None
        )
        ...
        if id_data == -1 and store_name and len(str(store_name)) > 0:
            ...
            company_id = CompanyOwner.add(
                ...
                order_status=(
                    OrderStatus.get_status(order_status)
                ...
                payment_status=(
                    PaymentStatus.get_status(payment_status)
                    if type(payment_status) == str
                    else payment_status
                ),
            ...
            if shipping_name and shipping_percentage and len(shipping_name) > 0:
                update_dict["shippings"] = {
                    ...
                    "percentage": (
                        shipping_percentage
                        if decimal.Decimal(shipping_percentage)
                        else 0
                    ),
                }
            if coupon_code and coupon_discount and len(coupon_code) > 0:
                update_dict["coupons"] = {
                    ...
                    "discount": (
                        coupon_discount if decimal.Decimal(coupon_discount) else 0
                    ),
                }
            ...
            if payment_name and payment_percentage and len(payment_name) > 0:
                update_dict["payments"] = {
                    ...
                    "percentage": (
                        payment_percentage if decimal.Decimal(payment_percentage) else 0
                    ),
                }

            CompanyOwner.update(updated=update_dict)
    ...
    def update_field_value(self, row, changed_column=None):
        def get_numeric_data(item):
            ...
            if user_data:
                try:
                    ...
                    return Decimal(
                        value.quantize(Decimal("0.00"), rounding=ROUND_HALF_UP)
                    )
        ...
        if self.rows_data.get(row, None):
            ...
            if (
                current_row_data["payment"] == payment_method.text()
                and current_row_data["coupon"] == discount_coupon.text()
                and payment_method
                and discount_coupon
            ):
        ...
        if (
            self.rows_data[row]["total_of_orders_discount"] != discount_coupon.text()
            or self.rows_data[row]["total_of_orders_discount"] <= 0
        ):
        ...
        if (
            ...
            or self.rows_data[row]["salla_total_discount"] <= 0
        ):
        ...
        if self.rows_data[row]["total_discount_value_pre_calculated"] is False:
            total_discount_value = (
                self.rows_data[row]["salla_total_discount"]
                + self.rows_data[row]["total_of_orders_discount"]
            )
